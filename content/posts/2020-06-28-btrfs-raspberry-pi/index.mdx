...
6. `sudo nano /boot/firmware/nobtcmd.txt`, then edit these two values to match: `root=UUID=520bcef5-5b86-4e6e-a0e7-e271db19a88d rootfstype=btrfs`
7. `sudo nano /etc/fstab`, again changing the following line: `UUID=520bcef5-5b86-4e6e-a0e7-e271db19a88d       /        btrfs  defaults,noatime`
8. `sudo reboot` - your device will close connection here, if you completed all the steps correctly you should be able to ssh back in within ~2 mins.
9. `sudo lsblk -f`
10. `sudo btrfs device add -f /dev/mmcblk0p2 /`
11. `sudo lsblk -f`
12. `sudo btrfs balance start -mconvert=raid1 /`
13. `sudo btrfs balance start -dconvert=raid1 /`
